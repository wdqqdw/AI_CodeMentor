...
- Tutor prompts: Encouraging Tutor v10 and Neutral Tutor v10, crossed with Fixed Low Scaffold, Fixed High Scaffold, and Adaptive Scaffold prompt modules. Prompt and deterministic fallback logic are task-aware: Boggle guidance uses grid-search language, while Word Ladder guidance uses implicit-graph and BFS language. Fixed Low may give one tiny local line or condition when code-level help is requested; Fixed High may give a 3-6 line incomplete local framework; Adaptive uses learner state plus recent failures and requests for ideas, hints, concrete help, line-level help, or code hints to choose support level. Complete-solution requests still trigger deterministic refusal, followed by scaffold-allowed local guidance.
- Tutor code context: enabled. The frontend sends line-numbered editor code to the backend; tutors may point to short line ranges. Local code hint requests keep this code context available, while complete solution requests still hide detailed context and refuse full answers.
...
- `backend/prompts/evaluations/scaffold_prompt_v10_2026-08-13.md`: latest scaffold usability and local-code-hint prompt checks.
